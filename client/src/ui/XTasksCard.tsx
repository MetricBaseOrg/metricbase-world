...
import { claimXTask, createXTask, fetchXTasks } from "../character/xTasksApi";
...
export function XTasksCard({ accessToken, onClaimed }: { accessToken: string | null; onClaimed?: () => void }) {
  ...
  // Nothing to show if there are no campaigns, the account is linked, and the
  // viewer isn't an admin (admins always see the card, to post campaigns).
  ...
  return (
    <section className="chibi-panel mb-dash-card">
      ...
      <div style={{ display: "grid", gap: 12 }}>
        {tasks?.map((t) => (
          <XTaskItem key={t.id} task={t} accessToken={accessToken} linked={linked}
            onClaimed={() => { setTasks((prev) => prev?.map((x) => x.id === t.id ? { ...x, claimed: true } : x) ?? prev); onClaimed?.(); }} />
        ))}
      </div>
      {admin && <AdminCreator accessToken={accessToken} onCreated={reload} />}
    </section>
  );
}

function AdminCreator({ accessToken, onCreated }: { accessToken: string; onCreated: () => void }) {
  const [open, setOpen] = useState(false);
  const [type, setType] = useState<"reply" | "quote">("reply");
  const [title, setTitle] = useState("");
  const [targetUrl, setTargetUrl] = useState("");
  const [description, setDescription] = useState("");
  const [hashtag, setHashtag] = useState("#MetricBase");
  ...
  const submit = async () => {
    ...
    const r = await createXTask(accessToken, { type, title, targetUrl, description, hashtag, points });
    setBusy(false);
    if (!r.ok) { setErr(r.error ?? "Failed"); return; }
    setTitle(""); setTargetUrl(""); setDescription(""); setOpen(false);
    onCreated();
  };
  ...
  return (
    <div style={{ marginTop: 12, display: "grid", gap: 8, borderTop: "2px dashed var(--chibi-outline-light)", paddingTop: 12 }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <button className="chibi-btn chibi-btn--ghost" type="button" onClick={() => setOpen(false)} style={{ padding: "8px 14px", fontSize: "0.8rem" }}>Cancel</button>
      </div>
...
function XTaskItem({ task, accessToken, linked, onClaimed }: {
  task: XTaskView; accessToken: string; linked: boolean; onClaimed: () => void;
}) {
  ...
  return (
    <div style={{ border: "2px solid var(--chibi-outline-light)", borderRadius: "var(--chibi-radius-sm)", padding: "12px 14px", background: "var(--chibi-cream)" }}>
