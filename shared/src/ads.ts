...
export interface AdSlot {
  ...
  /**
   * Player-World inventory: one slot whose winning creative shows in EVERY
   * opted-in player World rather than a single fixed zone. Each World's owner
   * earns from the impressions their own visitors generate — see
   * AD_WORLD_OWNER_SHARE.
   */
  playerWorlds?: boolean;
}
...
export const AD_SLOTS: AdSlot[] = [
  ...
  { id: "world_billboard", label: "Player Worlds Billboard", surface: "billboard", weight: 70, playerWorlds: true },
...
/** The shared slot every opted-in player World shows. */
export const AD_WORLD_SLOT_ID = "world_billboard";
...
export interface AdProgramPayload {
  ...
  invitedCount: number;
  /** Lifetime $BASE earned as a World OWNER from ads in their Worlds. Included
   *  in `lifetime`/`earnings` above — this breaks out how much came from
   *  owning inventory rather than from viewing. */
  worldOwnerLifetime: number;
  /** Impressions generated by visitors inside this player's Worlds. */
...
/**
 * A player World's owner earns this share of every impression generated inside
 * their World. With AD_PLAYER_SHARE that accounts for the whole impression:
 * World inventory pays out 100% (50% owner / 50% the viewing member) and the
 * platform keeps nothing, which is the incentive to build and publish a World.
 *
 * Two consequences to keep in mind when tuning:
 *  - Every accrual is a liability against the shared payout wallet, and World
 *    impressions add liabilities with no platform income to back them. The
 *    solvency guard in adService is therefore what keeps this honest.
 *  - Owner and viewer shares are never both paid to the same wallet (an owner
 *    idling in their own World earns the owner cut only) — see
 *    recordCampaignImpression.
 */
export const AD_WORLD_OWNER_SHARE = 0.5;
