...
export interface PlayerZoneMeta {
  ...
  guildOnly: boolean;
  /**
   * Owner opted this World into the ad revenue-sharing program. A billboard
   * then stands near the spawn showing the auction's Player Worlds creative,
   * and every impression it generates pays the owner AD_WORLD_OWNER_SHARE in
   * $BASE (the viewing member keeps the other half). Only published Worlds
   * serve ads — an unpublished World has no visitors to count.
   */
  adsEnabled: boolean;
  /** Lifetime $BASE (UI units) earned from ads in this World. Display only —
   *  the claimable balance lives in the ad program's member ledger. */
  adBaseEarned: number;
  /** Lifetime ad impressions generated by visitors inside this World. */
 ...
 */
/**
 * Where the ad billboard stands in a World that runs ads. Placement is
 * automatic rather than an editor prop: it has to be somewhere visitors
 * actually pass, and retro-fitting a placement step into every existing World
 * would mean nobody's World served an ad until they re-edited it.
 *
 * Picked deterministically from the spawn tile outward so it lands in view on
 * arrival, skipping tiles the owner has built on. The sign does NOT block
 * movement — dropping a new solid object into an already-built World could
 * wall off a doorway the owner laid out long before this existed.
 */
export function worldAdBillboardTiles(build: PlayerZoneBuild, gridSize: number): { x: number; y: number }[] {
  const spawn = safeBuildSpawnTile(build, gridSize);
  // Ring of candidates around the spawn, nearest first.
  const offsets = [
    { x: 2, y: 0 }, { x: -2, y: 0 }, { x: 0, y: 2 }, { x: 0, y: -2 },
    { x: 2, y: 2 }, { x: -2, y: -2 }, { x: 3, y: 0 }, { x: 0, y: 3 },
  ];
  for (const o of offsets) {
    const x = spawn.x + o.x;
    const y = spawn.y + o.y;
    if (x < 0 || y < 0 || x >= gridSize || y >= gridSize) continue;
    if (isBuildTileBlocked(build, x, y)) continue;
    return [{ x, y }];
  }
  return [];
}
...
export function playerZoneToConfig(record: PlayerZoneRecord): ZoneConfig {
  ...
  return {
    ...
    tiles: build.tiles,
    gridSize,
    // Only a published World serves ads (an unpublished one has no visitors),
    // and only when its owner has a wallet to be paid into.
    adBillboardTiles:
      record.adsEnabled && record.published && record.ownerWallet
        ? worldAdBillboardTiles(build, gridSize)
        : undefined,
  };
}
