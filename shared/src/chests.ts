...
export interface ChestTierDef {
  id: string;
  name: string;
  /** Fallback only — shown if the art 404s. Real art is `art` below. */
  emoji: string;
  /** Filename under client/public/assets/items (the client builds the URL, the
   * same split the fish species art uses). Source lives in assets/items and is
   * regenerated by `node scripts/process-items.mjs`. */
  art: string;
  blurb: string;
