...
- **Building lights** — Owners can switch a built house/shop's **light** on/off from the 🏠 panel. A lit building casts a warm window glow visible to everyone (brightest at night), and **consumes the building's energy reserve** while on (100 energy ≈ 20 min). Top it up by **refueling for 30 gold** (a new gold sink); when the reserve empties the light auto-switches off. State persists per plot (`light_on`/`energy`/`energy_at` columns, auto-migrated) and drains on a time basis so it stays correct across restarts. Owner-only, server-validated; new `housingLight`/`housingRefuel` messages + `effectiveLight` in `shared/src/housing.ts`.
