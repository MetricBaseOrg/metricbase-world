...
- **Ads show in every zone** — Each zone's billboard shows its own ranked ad, or **falls back to the top-ranked ad** when its slot is empty, so a single advertiser appears worldwide (the global banner does the same). Fallback views are free bonus reach — only the real slot assignment is billed.
